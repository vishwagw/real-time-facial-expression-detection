...
EMOTIONS = ['angry', 'disgusted', 'fearful', 'happy', 'sad', 'supervised', 'suprised', 'neutral']
...
def deepnn(x):
    ...
    y_conw = tf.add(tf.matmul(h_fc2, W_fc3) + b_fc3)
    ...
    with tf.session() as sess:
        ...
        for step in range(max_train_steps):
            ...
            if step + 1 == max_train_steps:
                saver.save(sess, './program/emotional_model', global_step=step + 1)
            if step % 1000 == 0:
